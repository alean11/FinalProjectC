...
import com.spring.wetre.chy.model.MessageVO;
import com.spring.wetre.jgh.model.MemberVO;
...
public class WebsocketEchoHandler extends TextWebSocketHandler {
	    ...
	    protected void handleTextMessage(WebSocketSession wsession, TextMessage message) 
	    	...
	    	if(loginuser != null) System.out.println("====> 웹채팅확인용 : 로그인ID : " + loginuser.getFk_userid());
	            ...
	            if (messageVO.getType().equals("all")) { // 채팅할 대상이 "전체" 일 경우 
	                if (!wsession.getId().equals(webSocketSession.getId())) {
                		if( loginuser != null && !"".equals(loginuser.getName())) {
                			webSocketSession.sendMessage(new TextMessage(wsession.getRemoteAddress().getAddress().getHostAddress() +" [" +loginuser.getName()+ "]" + " ▶ " + messageVO.getMessage()));  
                		}
                		else {
	    ...
	    public void afterConnectionClosed(WebSocketSession wsession, CloseStatus status) 
	        ...
	        for (WebSocketSession webSocketSession : connectedUsers) {
	            if (!wsession.getId().equals(webSocketSession.getId())) {
	            	...
	            	if(loginuser != null) {
	            		webSocketSession.sendMessage(new TextMessage(wsession.getRemoteAddress().getAddress().getHostAddress() +" [" +loginuser.getFk_userid()+ "]" + "님이 <span style='color: red;'>퇴장</span>했습니다.")); 
	            	}
	            	else {
